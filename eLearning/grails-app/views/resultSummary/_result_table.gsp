...
<g:if test="${results?.size() > 0 }">
	...
	<g:set var="totalMaxMark" value="${results?.sum { it?.exam?.maxMark } }"/>
	...
	<g:each in="${ results?.sort{it?.exam?.testNumber}}" var="result" status="i">
		<tr><td>${result?.exam?.testNumber }</td><td>${result?.exam?.maxMark }</td><td>${result?.mark }</td><td class="td-last">${String.format( '%.1f',result?.percentMark) }</td></tr>
	</g:each>
