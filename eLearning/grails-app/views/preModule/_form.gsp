...
<div class="fieldcontain ${hasErrors(bean: preModuleInstance, field: 'course', 'error')} required">
	<label for="course">
...
<div class="fieldcontain ${hasErrors(bean: preModuleInstance, field: 'current', 'error')} required">
	<label for="current">
		<g:message code="preModule.current.label" default="Current" />
		<span class="required-indicator">*</span>
	</label>
	<g:select id="current" name="current.id" from="${com.cland.elearning.Module.list()}" optionKey="id" required="" value="${preModuleInstance?.current?.id}" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: preModuleInstance, field: 'prerequisites', 'error')} ">
	...
	<g:select name="prerequisites" from="${com.cland.elearning.Module.list()}" multiple="multiple" optionKey="id" size="5" value="${preModuleInstance?.prerequisites*.id}" class="many-to-many"/>
</div>
