...
class ListComposer {
    ...
    private redraw(int activePage = 0) {
        ...
        def regionInstanceList = Region.createCriteria().list(offset: offset, max: max) {
            order('id','desc')
            if (idLongbox.value) {
                eq('id', idLongbox.value)
            }
        }
        paging.totalSize = regionInstanceList.totalCount
        listModel.clear()
        listModel.addAll(regionInstanceList.id)
    }

    private rowRenderer = {Row row, Object id, int index ->
        def regionInstance = Region.get(id)
        row << {
                a(href: g.createLink(controller:"region",action:'edit',id:id), label: regionInstance.id)
                label(value: regionInstance.name)
                label(value: regionInstance.country)
                hlayout{
                    toolbarbutton(label: g.message(code: 'default.button.edit.label', default: 'Edit'),image:'/images/skin/database_edit.png',href:g.createLink(controller: "region", action: 'edit', id: id))
                    toolbarbutton(label: g.message(code: 'default.button.delete.label', default: 'Delete'), image: "/images/skin/database_delete.png", client_onClick: "if(!confirm('${g.message(code: 'default.button.delete.confirm.message', default: 'Are you sure?')}'))event.stop()", onClick: {
                        Region.get(id).delete(flush: true)
                        listModel.remove(id)
                    })
                }
        }
